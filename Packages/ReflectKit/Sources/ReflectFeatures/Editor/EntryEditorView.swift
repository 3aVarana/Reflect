...
struct EntryEditorView: View {
    ...
    /// Invoked if the very last flush (from `.onDisappear`) fails. That failure can never be
    /// rendered in this view's own footer — the sheet is already gone by the time it happens
    /// — so the presenter wires this to an `.alert` of its own that outlives the sheet.
    private let onSaveFailure: (() -> Void)?

    init(mode: EditorMode, onSaveFailure: (() -> Void)? = nil) {
        self.mode = mode
    ...
    var body: some View {
        NavigationStack {
            ...
            .onAppear {
                if vm == nil {
                    let newVM = EntryEditorViewModel(mode: mode, context: modelContext)
                    newVM.onFinishFailure = onSaveFailure
                    newVM.load()
                    vm = newVM
                }
                isFocused = true
...
#Preview("Edit entry") {
    ...
    return EntryEditorView(mode: .edit(entry.id))
